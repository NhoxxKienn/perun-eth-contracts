...
import { MultiLedgerHarness, TrivialApp } from "../typechain-types";
...
describe("AdjudicatorCoordinate", function () {
    let adj: Adjudicator;
    ...
    async function coordinate(tx: Transaction): Promise<TransactionResponse> {
        const ch = new SignedChannel(tx.params, tx.state, tx.sigs);
        return adj.coordinate(
            ch.serialize(),
            [],
            [tx.coordSig],
            { from: adjAccount },
    ...
    async function coordinateWithSubchannels(ledgerChannel: Channel, subChannels: Channel[]): Promise<TransactionResponse> {
        let coordSigs: string[] = [];
        coordSigs[0] = await ledgerChannel.coordSigned();
        for (let i = 0; i < subChannels.length; i++) {
            coordSigs[i + 1] = await subChannels[i].coordSigned();
        }
    ...
    async function coordinateWithAssertions(channel: Channel, subChannels: Channel[]): Promise<void> {
        let coordSigs: string[] = [];
        coordSigs[0] = await channel.coordSigned();
        for (let i = 0; i < subChannels.length; i++) {
            coordSigs[i + 1] = await subChannels[i].coordSigned();
        }

        let res = await adj.coordinate(
            (await channel.signed()).serialize(),
            await Promise.all(subChannels.map(async ch => (await ch.signed()).serialize())),
            coordSigs,
            { from: adjAccount }
    ...
    async function assertCoordinate(receipt: TransactionReceipt, channel: Channel): Promise<void> {
        assertEventEmitted(receipt, 'ChannelUpdate', channel, DisputePhase.COORDINATED);
        await assertDisputePhase(channel.state.channelID, DisputePhase.COORDINATED);
    }
    ...
    describe("setup", () => {
        ...
        it("params setup", async () => {
            ...
            const coordinator = await signers[4].getAddress();
            coord = coordinator;
            ...
            params = new Params(appAddress, timeout, nonce, [parts[A], parts[B]], true, coordinator);
    ...
    describeWithBlockRevert("register and refute", () => {
        ...
        testsRegister.forEach(test => {
            it(test.desc, async () => {
                let tx = new Transaction(parts, balance, timeout, nonce, asset, backend, appAddress, coord);
                tx.state.version = "2";
        ...
        it("sign state", async () => {
            ...
            const tx = new Transaction(parts, balance, timeout, nonce, asset, backend, appAddress, coord);
            tx.state = state;
            tx.params = params;
            await tx.sign(parts);
    ...
    describeWithBlockRevert("virtual channels", () => {
        ...
        itWithBlockRevert("register with locked funds fails", async () => {
            let tx = new Transaction(parts, balance, timeout, nonce, asset, backend, zeroAddress, coord);
            tx.params.virtualChannel = true;
            tx.state.channelID = tx.params.channelID();
    ...
    describeWithBlockRevert("register, coordinate, conclude", () => {
        /* Create channel tree
         *      root
         *     /    \
         *   sub0   sub3
         ...
         * subchannel 1 final, others non-final
         * register
         * coordinate with subchannel states
         * conclude
         * withdraw
         */
        ...
        let ledgerChannel: Channel;
        let subchannels: Channel[];
        let coordAssets: Asset[];
        let coordBackends: number[];

        function createChannel(nonce: string, version: string, balances: BigNumberish[], ledger: boolean): Channel {
            const assets = coordAssets;
            let params = new Params(appAddress, timeout, nonce, parts, ledger, coord);
            let balancesByAsset = assets.map(() => [
                getUint(balances[0]).toString(),
                getUint(balances[1]).toString()
            ]);
            let outcome = new Allocation(
        ...
        before(async () => {
            coordAssets = [
                asset,
                new Asset(asset.chainID, ahAddress, zeroAddress)
            ];
            coordBackends = [1, 2];
        ...
        it("coordinate with subchannel states", async () => {
            await registerWithAssertions(ledgerChannel, subchannels);

            await advanceBlockTime(2 * timeout + 1);

            const res = await coordinateWithSubchannels(ledgerChannel, subchannels);

            const receipt = await res.wait();
            expect(receipt).to.not.be.null;
            if (!receipt) {
                throw new Error("coordinate transaction failed - receipt is null");
            }

            const parsed = receipt.logs.find(log => {
                try {
                    const p = adjInterface.parseLog(log);
                    return p && p.name === "ChannelUpdate";
                } catch {
                    return false;
                }
            });
            ...
            await Promise.all(subchannels.map(async ch =>
                assertDisputePhase(ch.state.channelID, DisputePhase.COORDINATED)
            ));
            await assertCoordinate(receipt, ledgerChannel);
        });


        itWithBlockRevert("conclude before coordination fails", async () => {
            const localSubchannels = Array.from({ length: 4 }).map(_ => {
                const nonce = newNonce();
                const version = nonce + nonce;
                const nonceAsNumber = Number(nonce);
                return createChannel(nonce, version, [ether(nonceAsNumber), ether(2 * nonceAsNumber)], false);
            });

            localSubchannels[1].state.isFinal = true;
            localSubchannels[0].state.outcome.locked = [
                toSubAlloc(localSubchannels[1].state),
                toSubAlloc(localSubchannels[2].state),
            ];

            const localLedgerChannel = createParentChannel(
                newNonce(),
                "10",
                [ether(10), ether(20)],
                [localSubchannels[0], localSubchannels[3]],
                true,
            );

            await registerWithAssertions(localLedgerChannel, localSubchannels);
            await advanceBlockTime(2 * timeout + 1);

            let res = concludeWithSubchannels(localLedgerChannel, localSubchannels);
            await expect(res).to.be.revertedWith("coordinated settlement required");
        });
    });

    describeWithBlockRevert("dispute, coordinate, conclude flow", () => {
        type ChannelTree = { ledgerChannel: Channel; subchannels: Channel[] };

        function buildChannelTree(
            assets: Asset[],
            backends: number[],
            ledgerVersion: string = "10"
        ): ChannelTree {
            let nonceCounter = 0;
            const newNonce = () => (++nonceCounter).toString();

            const createChannelLocal = (
                nonce: string,
                version: string,
            ...
            const toSubAllocLocal = (state: State): SubAlloc => {
                let assetTotals = state.outcome.balances.map(balancesForAsset =>
                    balancesForAsset.reduce((acc, val) =>
                        getUint(acc) + getUint(val),
                        0n
            ...
            const createParentChannelLocal = (
                nonce: string,
                version: string,
                balances: BigNumberish[],
                subchannels: Channel[],
                ...
                let channel = createChannelLocal(nonce, version, balances, ledger);
                channel.state.outcome.locked = subchannels.map(subchannel => toSubAllocLocal(subchannel.state));
                return channel;
            };

            const subchannels = Array.from({ length: 4 }).map(_ => {
                const nonce = newNonce();
                const version = nonce + nonce;
                const nonceAsNumber = Number(nonce);
                return createChannelLocal(nonce, version, [ether(nonceAsNumber), ether(2 * nonceAsNumber)], false);
            });

            subchannels[1].state.isFinal = true;
            subchannels[0].state.outcome.locked = [
                toSubAllocLocal(subchannels[1].state),
                toSubAllocLocal(subchannels[2].state),
            ];

            const ledgerChannel = createParentChannelLocal(
                newNonce(),
                ledgerVersion,
                [ether(10), ether(20)],
                [subchannels[0], subchannels[3]],
            ...
            return { ledgerChannel, subchannels };
        }

        async function fundLedgerChannel(ledgerChannel: Channel, subchannels: Channel[]): Promise<void> {
            const outcome = accumulatedOutcome(ledgerChannel, subchannels);
            await Promise.all(ledgerChannel.params.participants.map((user: Participant, userIndex: number) =>
                depositWithAssertions(ledgerChannel.state.channelID, user.ethAddress, outcome[userIndex]))
            );
        ...
        itWithBlockRevert("coordinate rejects invalid coordinator signature", async () => {
            const assets = [
                asset,
                new Asset(asset.chainID, zeroAddress, zeroAddress)
            ];
            const backends = [1, 2];
            ...
            const signers = await ethers.getSigners();
            const wrongCoord = await signers[5].getAddress();
            const coordSigs = [
                await ledgerChannel.state.coordSign(wrongCoord),
                ...(await Promise.all(subchannels.map(async ch => ch.coordSigned())))
            ];

            const res = adj.coordinate(
                (await ledgerChannel.signed()).serialize(),
                await Promise.all(subchannels.map(async ch => (await ch.signed()).serialize())),
                coordSigs,
                { from: adjAccount, gasLimit: 500_000 }
            );
            await expect(res).to.be.revertedWith("invalid coordinator signature");
        });

        itWithBlockRevert("coordinate rejects lower version than registered", async () => {
            const assets = [
                new Asset(asset.chainID, ahAddress, zeroAddress),
                new Asset(asset.chainID + 1, ahAddress, zeroAddress)
            ];
            const backends = [1, 2];
            ...
            ledgerChannel.state.version = "9";
            const res = coordinateWithSubchannels(ledgerChannel, subchannels);
            await expect(res).to.be.revertedWith("invalid version");
        });

        itWithBlockRevert("coordinate updates dispute version on higher state", async () => {
            const assets = [
                new Asset(asset.chainID, zeroAddress, zeroAddress),
                new Asset(asset.chainID, zeroAddress, zeroAddress)
            ];
            ...
            ledgerChannel.state.version = "11";
            const res = await coordinateWithSubchannels(ledgerChannel, subchannels);
            const receipt = await res.wait();
            if (!receipt) {
                throw new Error("coordinate transaction failed - receipt is null");
            ...
            const dispute = await adj.disputes(ledgerChannel.state.channelID);
            expect(dispute.version).to.equal(11n);
            await assertCoordinate(receipt, ledgerChannel);
        });

        itWithBlockRevert("coordinate rejects non-multi-ledger state", async () => {
            const assets = [asset];
            const backends = [1];
            const { ledgerChannel, subchannels } = buildChannelTree(assets, backends, "10");
            await fundLedgerChannel(ledgerChannel, subchannels);
            ...
            const res = coordinateWithSubchannels(ledgerChannel, subchannels);
            await expect(res).to.be.revertedWith("incorrect phase");
        });

        itWithBlockRevert("coordinate then conclude sets holdings for base asset", async () => {
            const assets = [
                asset,
                new Asset(asset.chainID, zeroAddress, zeroAddress)
            ];
            ...
            const coordRes = await coordinateWithSubchannels(ledgerChannel, subchannels);
            const coordReceipt = await coordRes.wait();
            if (!coordReceipt) {
                throw new Error("coordinate transaction failed - receipt is null");
            }

            const concludeRes = await concludeWithSubchannels(ledgerChannel, subchannels);
            const concludeReceipt = await concludeRes.wait();
            if (!concludeReceipt) {
                throw new Error("conclude transaction failed - receipt is null");
            }

            await assertConclude(concludeReceipt, ledgerChannel, subchannels, false);
        });

        itWithBlockRevert("coordinate rejects virtual channels", async () => {
            const assets = [
                new Asset(asset.chainID, zeroAddress, zeroAddress),
                new Asset(asset.chainID, zeroAddress, zeroAddress)
            ];
            ...
            // Create a virtual channel (ledgerChannel: false)
            let virtualParams = new Params(appAddress, timeout, "0x99", parts, false, coord);
            let balancesByAsset = assets.map(() => [
                getUint(ether(10)).toString(),
                getUint(ether(20)).toString()
            ]);
            let virtualOutcome = new Allocation(assets, backends, balancesByAsset, []);
            let virtualState = new State(virtualParams.channelID(), "10", virtualOutcome, "0x00", false);
            let virtualChannel = new Channel(virtualParams, virtualState);

            // Don't register - just try to coordinate directly
            const coordSigs = [await virtualChannel.state.coordSign(coord)];

            const res = adj.coordinate(
                (await virtualChannel.signed()).serialize(),
                [],
                coordSigs,
                { from: adjAccount, gasLimit: 500_000 }
            );
    ...
    describeWithBlockRevert("dispute, coordinate, conclude with virtual channel", () => {
        ...
        itWithBlockRevert("coordinate and conclude with virtual subchannel succeeds", async () => {
            const assets = [
                new Asset(asset.chainID, zeroAddress, zeroAddress),
                new Asset(asset.chainID, zeroAddress, zeroAddress)
            ];
            ...
            const virtualParams = new Params(zeroAddress, timeout, "0x21", parts, false, coord);
            const virtualBalances = assets.map(() => [
                getUint(ether(5)).toString(),
                getUint(ether(7)).toString()
            ]);
            virtualParams.virtualChannel = true;
            const virtualOutcome = new Allocation(assets, backends, virtualBalances, []);
            const virtualState = new State(
                virtualParams.channelID(),
                "3",
                virtualOutcome,
                "0x00",
                false
            );
            const virtualChannel = new Channel(virtualParams, virtualState);

            const ledgerParams = new Params(appAddress, timeout, "0x22", parts, true, coord);
            const ledgerBalances = assets.map(() => [
                getUint(ether(10)).toString(),
                getUint(ether(20)).toString()
            ]);
            const ledgerOutcome = new Allocation(assets, backends, ledgerBalances, []);
            const ledgerState = new State(
                ledgerParams.channelID(),
                "10",
                ledgerOutcome,
                "0x00",
                false
            );
            const virtualTotals = virtualOutcome.balances.map(balancesForAsset =>
                balancesForAsset.reduce((acc, val) =>
                    getUint(acc) + getUint(val),
                    0n
                )
            );
            ledgerState.outcome.locked = [
                new SubAlloc(
                    virtualState.channelID,
                    virtualTotals.map(total => total.toString()),
                    [0, 1]
                )
            ];
            const ledgerChannel = new Channel(ledgerParams, ledgerState);

            const subchannels = [virtualChannel];
            await fundLedgerChannel(ledgerChannel, subchannels);
            await registerWithAssertions(ledgerChannel, subchannels);
            await advanceBlockTime(2 * timeout + 1);
    ...
    describeWithBlockRevert("progress", async () => {
        ...
        before(async () => {
            ...
            let tx2 = new Transaction(parts, balance, timeout, "0x02", asset, backend, appAddress, coord);
            tx2.state.version = "0";
        ...
        testsProgress.forEach(test => {
            it(test.desc, async () => {
                let txOld = new Transaction(parts, balance, timeout, nonce, asset, backend, appAddress, coord);
        ...
        function testWithModifiedOldState(description: string, prepare: any) {
            itWithBlockRevert(description, async () => {
                ...
                let nonce = "1";
                let tx1 = new Transaction(parts, balance, timeout, nonce, asset, backend, appAddress, coord);
                tx1.state.version = "1";
                prepare(tx1);
                ...
                let tx2 = new Transaction(parts, balance, timeout, nonce, asset, backend, appAddress, coord);
                tx2.state.version = "2";
                await tx2.sign(parts);
                await advanceBlockTime(timeout + 1);
                let actorIdx = 0;
